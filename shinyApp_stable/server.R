...
PYTHON_DEPENDENCIES = c("pyreadr","tables","scanpy")
...
shinyServer(function(input, output, session) {
    ...
    sc1c1oup <- reactive({
        scVioBox(sc1c1Data(), sc1conf, input$sc1c1inp1, input$sc1c1inpg2,input$sc1c1inp3,
                 ...
                 input$sc1c1err,input$sc1c1siz, input$sc1c1fsz,
    ...
    sc1d1oup <- eventReactive(input$sc1d1update,{
        scBubbHeat(sc1d1Data(),input$sc1d1inp, input$sc1d1inp2, sc1gene, input$sc1d1zscore,input$sc1d1row,
                   ...
                   input$sc1d1leg, input$sc1d1legpos, input$sc1d1xlim, input$sc1d1ylim)
    })
    ...
    sc1f2adata <- eventReactive(input$sc1f2update,{
        NeedReRun <- !(.checkIfIdentical(inpsub1_2 = input$sc1f2sub1_2, inpsub2_2 = input$sc1f2sub2_2, inpsub3_2 = input$sc1f2sub3_2,
                                         inpGrp = input$sc1f2grp,
                                         file.name = "tempData/rank_genes_groups.csv")) & input$sc1f2plt != "Correlation Matrix"
        if(NeedReRun){
            print("NeedReRun!")
    ...
    output$sc1f2oup.png <- downloadHandler(
        filename = function() { paste0(Sys.Date(),"-",input$sc1f2plt,"_",input$sc1f2grp,".png") },
    ...
    sc1f2DEGs <- reactive({
        return(loadAllDEGs(sc1f2adata(),input$sc1f2grp, input$sc1f2cutp, input$sc1f2cutpval,
                           input$sc1f2cutfc, input$sc1f2cutpct,
    ...
    sc1g1DEGs <- reactive({
        print("loadAllDEGs!")
        return(loadAllDEGs(sc1g1adata(),input$sc1g1grp, input$sc1g1cutp, input$sc1g1cutpval,
